...
var packageDir = tusk.getDirectory().join( 'packages');
parser.option('-p', '--packageDir', 'packageDir' )
    .def(packageDir)
    .help('specify the directory path to install packages relative to sea root, otherwise use "' + packageDir + '".' )
	.set();
/*
...
exports.install = function (options, names) {
    ...
    missing.forEach(function (name) {
        ...
        try {
            var targetPath = fs.path( options.packageDir ).join( name);
